...
def max_pooling(inp, pooling_shape, stride_shape):
        x_shape = inp.shape[0]-pooling_shape[0]+1
        y_shape = inp.shape[1]-pooling_shape[1]+1
        output = np.zeros((np.arange(0, x_shape, stride_shape[0]).shape[0],
                          np.arange(0, y_shape, stride_shape[1]).shape[0]))
        local_max = np.zeros((np.arange(0, x_shape, stride_shape[0]).shape[0],
                          np.arange(0, y_shape, stride_shape[1]).shape[0], 
                          2))
        for i in range(0, x_shape, stride_shape[0]):
            for j in range(0, y_shape, stride_shape[1]):
                # init
                x1= i
                x2 = int(x1 + pooling_shape[0])
                y1 = j
                y2 = int(y1 + pooling_shape[1])
                # calculate
                output[int(i/stride_shape[0]), int(j/stride_shape[1])] = np.max(inp[x1:x2, y1:y2])
                x, y = np.unravel_index(inp[x1:x2, y1:y2].argmax(), inp[x1:x2, y1:y2].shape)
                x += x1
                y += y1
                local_max[int(i/stride_shape[0]), int(j/stride_shape[1])] = (int(x), int(y))
